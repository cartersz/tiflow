...
// captureTableSet is a set of table ID that is in a capture.
type captureTableSet map[model.TableID]schedulepb.TableState

type replicationManager struct {
	tables       map[model.TableID]ReplicationSet
	runningTasks map[model.TableID]*scheduleTask
	captures     map[model.CaptureID]captureTableSet
}

func (s *replicationManager) captureTableSets() map[model.CaptureID]captureTableSet {
	return s.captures
}
